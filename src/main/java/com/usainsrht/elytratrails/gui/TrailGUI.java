...
public class TrailGUI {
    ...
    private static final int FILLER_ROW_START = TRAIL_SLOTS; // slot 45
    ...
    private static String categoryTitle(TrailCategory category) {
        return switch (category) {
            case ELYTRA -> ChatColor.AQUA + "" + ChatColor.BOLD + "Elytra Trails";
            case PLAYER -> ChatColor.GREEN + "" + ChatColor.BOLD + "Player Trails";
            case ARROW  -> ChatColor.GOLD  + "" + ChatColor.BOLD + "Arrow Trails";
        };
    }

    private static Material categoryFillerMaterial(TrailCategory category) {
        return switch (category) {
    ...
    public void open(Player player, int page, TrailCategory category) {
        ...
        String title = categoryTitle(category) + ChatColor.GRAY + " (" + (page + 1) + "/" + totalPages + ")";
        ...
        ItemStack filler = createFiller(categoryFillerMaterial(category));
        for (int i = FILLER_ROW_START; i < SIZE; i++) {
            inv.setItem(i, filler);
        }

        // Back button (slot 45)
        inv.setItem(45, createItem(Material.ARROW, ChatColor.LIGHT_PURPLE + "← Back to Cosmetics"));

        // Previous page button
        if (page > 0) {
            inv.setItem(46, createItem(Material.PAPER, ChatColor.YELLOW + "← Previous Page"));
        }
        ...
        if (page < totalPages - 1) {
            inv.setItem(52, createItem(Material.PAPER, ChatColor.YELLOW + "Next Page →"));
        }
    ...
    private ItemStack createTrailItem(Trail trail, UUID uuid, String activeTrailId,
        ...
        if (trail.getCategory() == TrailCategory.PLAYER) {
            lore.add(ChatColor.GRAY + "Mode: " + ChatColor.WHITE + trail.getPlayerTrailMode().name());
        }
        lore.add("");
